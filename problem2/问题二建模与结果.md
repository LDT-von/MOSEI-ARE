...
以下结果在 RTX 3060 Laptop GPU 上用 seed=42 取得。所有 checkpoint 大小约 19.6–19.9 MB。
...
| C   | 9         | 0.3776         | 0.4424  | 0.4644  | 0.7602   | 0.7559   |
...
| C   | clean   | 0.492 | 0.449   | 0.839 | 0.393   |
| C   | mixed   | 0.490 | 0.446   | 0.845 | 0.368   |
...
1. **B 组在 clean F1 上略优于 A/C**（0.4658 vs 0.4596/0.4424），但 C 组在 mixed F1 上最优（0.4644），说明跨模态补偿在低质量输入上有一定作用。
...
4. **单种子结果**（seed=42）：论文应补充至少 3 个种子的均值和标准差。
